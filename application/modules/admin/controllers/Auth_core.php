...
class Auth_core extends CI_Controller {
	...
	function currentpass_check($str)
	{
		...
		if ($res<=0)
		{
			$this->form_validation->set_message('currentpass_check', lang_key_admin('current_password_not_matched'));
			return FALSE;
		}
		else
		{
	...
	function recoverpassword()
	{
		...
		else
		{
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key_admin('email_sent_inbox').'</div>');
	...
	function useremail_check($str)
	{
		...
		if ($res<=0)
		{
			$this->form_validation->set_message('useremail_check', lang_key_admin('email_not_matched'));
			return FALSE;
		}
		else
		{
	...
	function resetpassword($user_name='',$recovery_key='')
	{
		...
		{
			$this->session->set_flashdata('msg', '<div class="alert alert-block">'.lang_key_admin('password_recovery_link_invalid').'</div>');
			redirect(site_url('admin/auth/forgotpass'));
		}
	}
	#*************************************************#
	...
	function login()
	{
		...
		{					
			...
			{
				$data = array('error'=>'<div class="alert alert-danger" style="margin-top:10px;">'.lang_key_admin('account_not_confirmed').'</div>');
				 load_admin_view('login_view',$data);				
			}
		}
		else
		{
			$data = array('error'=>'<div class="alert alert-danger" style="margin-top:10px;">'.lang_key_admin('login_failed').'</div>');
			 load_admin_view('login_view',$data);
		}
	}
    ...
    function useremail_check_signup($str)
    {
        ...
        if ($res > 0)
        {
            $this->form_validation->set_message('useremail_check_signup', lang_key_admin('email_allready_in_use'));
            return FALSE;
        }
        else
        {
    ...
    function username_check_signup($str)
    {
        ...
        if ($res > 0)
        {
            $this->form_validation->set_message('username_check_signup', lang_key_admin('username_allready_in_use'));
            return FALSE;
        }
        else
        {
	...
	function signup()
	{
		...
		else
		{
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key_admin('confirm_email_send').'</div>');

            redirect(site_url('admin/auth'));
		}
	}

    public function send_confirmation_email($data=array('username'=>'sc mondal','useremail'=>'webhelios','confirmation_key'=>'1234'))
...

    {
...

        {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key_admin('email_confirmed').'</div>');
...

        {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-danger">'.lang_key_admin('email_confirmed_failed').'</div>');
