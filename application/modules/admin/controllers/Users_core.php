...
class Users_core extends CI_Controller {
	...
	public function all($start='0')
	{
        ...
        $data['title'] 		= lang_key_admin('users');
	...
	public function detail($id)
	{
        ...
        $data['title'] 		= lang_key_admin('user_profile');
	...
	function create()
	{
		...
		$data['title'] 		= lang_key_admin('create_user');
	...
	public function useremail_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('useremail_check', lang_key_admin('email_allready_in_use'));
			return FALSE;
		}
		else
		{
	...
	public function username_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('username_check', lang_key_admin('username_allready_in_use'));
			return FALSE;
		}
		else
		{
	...
	function add()
	{
		...
		$this->form_validation->set_rules('user_email',lang_key_admin('user_email'), 'required|xss_clean|callback_useremail_check');
		$this->form_validation->set_rules('user_name',lang_key_admin('username'), 	'required|xss_clean|callback_username_check');
		$this->form_validation->set_rules('first_name',lang_key_admin('first_name'), 'required|xss_clean');
		$this->form_validation->set_rules('last_name',lang_key_admin('last_name'), 	'required|xss_clean');
		$this->form_validation->set_rules('user_type',lang_key_admin('user_type'), 	'required|xss_clean');
		$this->form_validation->set_rules('password', lang_key_admin('password'), 'required|matches[confirm_password]|min_length[5]|xss_clean');
		$this->form_validation->set_rules('confirm_password',lang_key_admin('confirm_password'), 'required|xss_clean');
		$this->form_validation->set_rules('gender',lang_key_admin('gender'), 	'required|xss_clean');

		$user_type = $this->input->post('user_type');
		// if($user_type==2)
		// 	$this->form_validation->set_rules('company_name','Company name', 'required|xss_clean');

		// if(get_settings('global_settings','enable_pricing','No')=='Yes')
...

		{
			...
			if(get_settings('global_settings','enable_pricing','No')=='Yes')
			{
				...
				$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key_admin('user_created').'</div>');
