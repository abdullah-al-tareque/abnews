...
class Page_core extends CI_Controller {
	...
	{
		$values = array();
		$values['title'] 	= lang_key_admin('new_page');
		$data['title'] 		= lang_key_admin('new_page');
		if($id!='')
		{
			$data['title'] 			= lang_key_admin('edit_page');
			$values['title'] 		= lang_key_admin('edit_page');
	...
	public function is_valid_alias($str)
	{
		...
		if ($res > $val)
		{
			$this->form_validation->set_message('is_valid_alias', lang_key_admin('unique_alias'));
			return FALSE;
		}
		else
		{
		...
		else
		{
			...
			else
			{
				if($this->input->post('action_type')=='update')
				{
					...
					$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key_admin('page_updated').'</div>');
				}
				else
				{
	...
	public function all($start='0')
	{
        ...
        $data['title'] = lang_key_admin('all_pages');
	...
	{
        $data['title'] = lang_key_admin('menu');
