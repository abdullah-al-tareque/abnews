...
if ( ! function_exists('default_lang_admin'))
{
	function default_lang_admin()
	{
		if(is_installed('return')!='no')
		{
			if(defined('DEFAULT_LANG'))
			...
			{	
				if(is_installed('return')=='yes')
				{
					...
					if($query->num_rows()>0)
					{
						$row = $query->row();
						$settings = json_decode($row->values);
						$default_lang = (isset($settings->admin_lang))?$settings->admin_lang:$settings->site_lang;			
					}
					else
						$default_lang = 'en';
...
if ( ! function_exists('get_author_url'))
{
	...
	{		
		//return 'https://skywebit.com/app/envato/';
	}
}
...
if ( ! function_exists('get_user_type_by_id'))
{
	function get_user_type_by_id($id)
	{
		...
		elseif($id==2)
			return lang_key('moderator');
		elseif($id==3)
			return lang_key('general');
	}
}
...
if ( ! function_exists('is_generaluser'))
{
	function is_generaluser()
	{
		$CI = get_instance();
...
if ( ! function_exists('lang_key_admin'))
{
	function lang_key_admin($key='')
	{
		if(defined('LANG_ARRAY'))
		{
			$lang = (array)json_decode(constant('LANG_ARRAY'));
...
if ( ! function_exists('if_possible_to_grab'))
{
	function if_possible_to_grab()
	{
